...
public:
    ...
    bool readFromBMP(const int width, const int height, unsigned char* buffer);

    /** 压缩到jpg文件中，quality_scale表示质量，取值范围(0,100), 数字越大压缩比例越高*/
    bool encodeToJPG(const char* fileName, int quality_scale, Mat* M, const int mode);
...
private:
    ...
    unsigned char fwbuffer[fwbuff_size+4];                     //fwrite buffer
    int buffcount;                                           //buffer counter
    ...
    void _convertColorSpace(int xPos, int yPos, signed char* yData, signed char* cbData, signed char* crData);
    void _foword_FDC(const signed char* channel_data, short* fdc_data, float* dct_data);
    ...
    void _write_bitstring_(const BitString* bs, int counts, unsigned short& neword, int& newordPos, FILE* fp);
    void _write_(const void* p, int byteSize, FILE* fp);
    void _write_special_byte_(unsigned char value, FILE* fp);
    void _write_special_word_(unsigned short value, FILE* fp);
    void push2buff(unsigned short value, FILE* fp, int flag);
};